...
class PCATest extends Specification {
  ...
  private def matrix(vs: VectorSpace, values: Array[Double]): vs.SquareMatrix =
    vs.squareMatrix(values.toIndexedSeq)
  ...
  "Iterations method" should {
    "not fail on one simple case" in {
      ...
      method.maxEigenValue(R2)(m) match {
        case Some((value: Double, vec: R2.Vector, _)) =>
          value === 100.0
        ...
        case trash => failure(s"Expected eigenvalues, got $trash")
      }
      ok
    ...
    "produce first eigenvector for a 3x3" in {
      ...
      method.maxEigenValue(R3)(m) match {
        case Some((value: Double, vec: R3.Vector, _)) =>
    ...
    "produce two eigenvectors for a 3x3 in many small steps" in {
      ...
      method.maxEigenValue(R3)(m) match {
        case Some((value1: Double, vector1: R3.Vector, nIter1)) =>
          ...
          val newBasis: R3.UnitaryMatrix =
            R3.unitaryMatrix(R3.buildOrthonormalBasis(vector1).map(_.copy).toIndexedSeq)
          ...
          method.maxEigenValue(R2)(submatrix) match {
            case Some((value2: Double, vector2, _: Int)) =>

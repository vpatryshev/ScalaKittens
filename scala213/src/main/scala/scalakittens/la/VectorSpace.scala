...
case class VectorSpace(dim: Int) { space =>
  ...
  def projectToHyperplane(v: Vector): hyperplane.Vector =
    hyperplane.OnFunction(i => v(i+1)).asInstanceOf[hyperplane.Vector] // weird, why cast in 2.13?

  def projectToHyperplane(m: SquareMatrix): hyperplane.SquareMatrix =
    hyperplane.squareMatrix((i, j) => m(i + 1, j + 1)).asInstanceOf[hyperplane.SquareMatrix] // weird, why cast in 2.13?
  ...
  def mult(v1: Vector, v2: Vector): Double = {
    v1.range.foldLeft(0.0)((s, i) => s + v1(i) * v2(i))
  }
  ...
  /**
    ...
    *
    * Created by Vlad Patryshev on 5/14/17.
    */
...
//    def space = VectorSpace.this
    ...
    private lazy val _l2 = Norm.l2(range map this)
  ...
  class OnArray(private[la] val data: Array[Double]) extends MutableVector {
    ...
    override def *(other: Vector): Double = {
      other match {
        ...
        case _ =>
          range.foldLeft(0.0)((s, i) => s + data(i) * other(i))
      }
    }
  ...
  case class OnFunction(f: Int => Double) extends Vector {
    override def apply(i: Int): Double = f(i)
  }
  ...
  /**
    ...
    */
  def const(value: Double): OnFunction = OnFunction(_ => value)
  ...
  def inf(vectors: IterableOnce[Vector]): Vector = {
    ...
    vectors.iterator foreach acc.decreaseBy
    acc
  }
  ...
  def sup(vectors: IterableOnce[Vector]): Vector = {
    ...
    vectors.iterator foreach acc.increaseBy
    acc
  }
  ...
  def *[Domain <: VectorSpace, Codomain <: VectorSpace](
  ...
  // TODO: add a factory that takes an array
  ...
  trait SquareMatrix extends LocalMatrix {
    ...
    def isUnitary(precision: Double): Boolean = {
      val delta: Iterator[Double] = (this * transpose - UnitMatrix).iterator
      Norm.l2(delta) <= precision
    }
      ...
      v match {
        ...
        case _ =>
          val data = rowRange map {
            i => v.indices.foldLeft(0.0)((s, j) => s + this(i, j)*v(j))
          } toArray
  ...
  /**
    ...
    *
    * Created by Vlad Patryshev on 5/25/17.
    */
  case class Basis(center: Vector, rotation: UnitaryMatrix) extends (Vector => Vector) {
    ...
    def apply(v: Vector): Vector = transform(v)
  ...
  object Basis {
    ...
    def apply(center: Vector, basisVectors: Array[MutableVector]): Basis = {
      ...
      val matrix: UnitaryMatrix = unitaryMatrix(basisVectors.toList)
    ...
    def apply[V <: Vector](basisVectors: Array[V]): Basis = {
      ...
      val matrix: UnitaryMatrix = new ColumnMatrix[space.type](space, columns.toIndexedSeq) with UnitaryMatrix
...
object Spaces {
  ...
  case class inside(space: VectorSpace) {
    def apply[T](f: VectorSpace => T): T = space.run(f)
  }

  def mult(s: VectorSpace)(v1: s.Vector, v2: s.Vector): Double =
    v1.range.foldLeft(0.0) ((s, i) => s + v1(i) * v2(i))
  
  lazy val R0: VectorSpace = VectorSpace(0)
  lazy val R1: VectorSpace = VectorSpace(1)
  lazy val R2: VectorSpace = VectorSpace(2)
  lazy val R3: VectorSpace = VectorSpace(3)
  lazy val R4: VectorSpace = VectorSpace(4)
  lazy val R5: VectorSpace = VectorSpace(5)
  lazy val R6: VectorSpace = VectorSpace(6)
  lazy val R7: VectorSpace = VectorSpace(7)
  lazy val R8: VectorSpace = VectorSpace(8)
  lazy val R9: VectorSpace = VectorSpace(9)
  lazy val R10: VectorSpace = VectorSpace(10)
  lazy val R11: VectorSpace = VectorSpace(11)
  lazy val R12: VectorSpace = VectorSpace(12)
  lazy val R13: VectorSpace = VectorSpace(13)
  lazy val R14: VectorSpace = VectorSpace(14)
  lazy val R15: VectorSpace = VectorSpace(15)
  lazy val R16: VectorSpace = VectorSpace(16)
  lazy val R17: VectorSpace = VectorSpace(17)
  lazy val R18: VectorSpace = VectorSpace(18)
  lazy val R19: VectorSpace = VectorSpace(19)
  lazy val R20: VectorSpace = VectorSpace(20)
  lazy val R21: VectorSpace = VectorSpace(21)
  lazy val R22: VectorSpace = VectorSpace(22)
  lazy val R23: VectorSpace = VectorSpace(23)
  lazy val R24: VectorSpace = VectorSpace(24)
  lazy val R25: VectorSpace = VectorSpace(25)
  lazy val R26: VectorSpace = VectorSpace(26)
  lazy val R27: VectorSpace = VectorSpace(27)
  lazy val R28: VectorSpace = VectorSpace(28)
  lazy val R29: VectorSpace = VectorSpace(29)
  lazy val R30: VectorSpace = VectorSpace(30)
  lazy val R31: VectorSpace = VectorSpace(31)
  lazy val R32: VectorSpace = VectorSpace(32)
  lazy val R33: VectorSpace = VectorSpace(33)
  lazy val R34: VectorSpace = VectorSpace(34)
  lazy val R35: VectorSpace = VectorSpace(35)
  lazy val R36: VectorSpace = VectorSpace(36)
  lazy val R100: VectorSpace = VectorSpace(100)
}

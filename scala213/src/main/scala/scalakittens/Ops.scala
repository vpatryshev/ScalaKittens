...
trait Ops {
  ...
  implicit class Ternary(b: => Boolean) {
    def ?[T](x: => T): Object {def |(y: => T): T} = new {
  ...
  def union[T](sets: IterableOnce[Set[T]]): Set[T] = sets.iterator.foldLeft(Set.empty[T])(_ ++ _)
  ...
  /**
    ...
    */
  def groupByRelationship[T](p: (T, T) => Boolean)(xs: Iterable[T]): List[List[T]] = {
    val (seg, acc) = xs.foldLeft((List[T](), List[List[T]]())) {
  ...
  def spendNotMoreThan[T](time: Duration, extraTimePercent: Int = 1): Starter[T] =
    new Starter[T](time, extraTimePercent, _ => ()) {

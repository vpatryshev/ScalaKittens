...
trait TimeReader {
  ...
  private def maxTimestamp = currentTime + MILLIS_IN_A_YEAR // 1 year ahead, do we need this date?
  ...
  def jodaToday: DateTime =
    jodaNow.withHourOfDay(12).withMinuteOfHour(0).withSecondOfMinute(0).withMillisOfSecond(0)

  def jodaTomorrow: DateTime =
    jodaNow.plusDays(1).withHourOfDay(12).withMinuteOfHour(0).withSecondOfMinute(0).withMillisOfSecond(0)
...
object DateAndTime extends DateAndTime with TimeReader {
  ...
  val MMddyy: Regex = """(\d{1,2})/ ?(\d{1,2})/(\d{2})\b.*""".r
  val xxyyzz: Regex = """(\d{1,2})-(\d{1,2})-(\d{2})\b.*""".r
  val MMddyyyy: Regex = """(\d{1,2})[\/-](\d{1,2})[\/-](\d{4}).*""".r
  val Mmddyyyy: Regex = """(\d{2})(\d{2})(\d{4}).*""".r
  val MMMddyyyy: Regex = """(\w{3,}) (\d{1,2}),? (\d{4})""".r
  val YyyyMMdd: Regex = """(\d{4})-(\d{2})-(\d{2})""".r
